...
class TrainModel(BaseModel):
    ...
    def _validation_loss(self, x_train, x_test):
        ...
        num_test_labels=tf.cast(tf.math.count_nonzero(mask),dtype=tf.float32) # count the number of non zero values
    ...
    def train(self, x):
        ...
        num_train_labels=tf.cast(tf.math.count_nonzero(mask),dtype=tf.float32) # number of non zero values in the training set
        ...
        if self.FLAGS.l2_reg==True:
            l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in tf.compat.v1trainable_variables()])
            MSE_loss = MSE_loss +  self.FLAGS.lambda_ * l2_loss
        
        train_op=tf.compat.v1.train.AdamOptimizer(self.FLAGS.learning_rate).minimize(MSE_loss)
        RMSE_loss=tf.sqrt(MSE_loss)
