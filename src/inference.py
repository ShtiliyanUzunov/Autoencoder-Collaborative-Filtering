...
tf.compat.v1.flags.DEFINE_string('checkpoints_path', os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'checkpoints/')), 
                           'Path for the test data.')

tf.compat.v1.flags.DEFINE_string('export_path_base', os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'model-export/')), 
...
def run_inference():
    ...
    with inference_graph.as_default():
        ...
        input_data=tf.compat.v1.placeholder(tf.float32, shape=[None, 3952])
